...
def rotate_no_shaw(pcdArr, intensity, semantics, labelInstance, pcdArrAsset, details, rotation, modelPredictionsScene):
    import time
    ...
    while (len(degrees) > 0 and attempts < config.ADD_PLACEMENT_ATTEMPT_LIMIT and not success):
        ...
        # Check on ground
        mask_list = rule["06"][learning_map_inv[details["typeNum"]]]
        print(mask_list)
        maskGround = np.zeros_like(semantics, dtype=bool)
        for mask in mask_list:
            maskGround |= (semantics == mask)
        if (details["assetType"] == "traffic-sign"):
            success = pointsAboveGround(pcdArrAssetNew, pcdArr, maskGround) 
        else:
            success = pointsAboveGround(pcdArrAssetNew, pcdArr, maskGround) or pointsWithinDist(pcdArrAssetNew, 5) 
        ...
        if (not success):
            # print("Points not on correct ground")
            details["issue"] = "Points not on correct ground"
        else:
            # On correct ground
    ...
    return success, pcdArrAssetNew, (pcdArr, intensity, semantics, labelInstance), details, modelPredictionsScene
...
def getPointsOnLine(p1, p2, count):
    ...
    pointsOnLine = np.zeros((len(xVals), 3), dtype=float)
...
def pointsAboveGround(points, scene, maskGround):
    ...
    # pointspoint= o3d.geometry.PointCloud()
    # pointspoint.points = o3d.utility.Vector3dVector(pointsCopy)

    # o3d.visualization.draw_geometries([pcdScene,pointspoint])
...
# def pointsAboveGround(points, scene, maskGround):
#     """
#     Checks that a sufficient number of points exist above the ground.
#     """
#     print(maskGround)
#     from scipy.spatial import cKDTree
#     pcdArrGround = scene[maskGround, :]

#     # Create KDTree for ground points
#     kdtree = cKDTree(pcdArrGround)

#     # Calculate distances from points to the nearest ground point
#     distances, _ = kdtree.query(points, k=1)

#     # Check how many points are above the ground based on the distance threshold
#     points_above_ground = distances > 0.1

#     # Check if at least a third of the points are above ground
#     return np.sum(points_above_ground) >= (len(points_above_ground) / config.ADD_OVER_GROUND)
