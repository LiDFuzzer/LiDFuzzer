...
def changeIntensity(pcdArrAsset, pcdArr, pcdArrintensity, intensity):
    pcdArrAsset_tuples = [tuple(point) for point in pcdArrAsset]

    pcdArr_tuples = [tuple(point) for point in pcdArr]

    pcdArr_tuples_np = np.array(pcdArr_tuples)

    indices = np.where(np.isin(pcdArr_tuples_np, pcdArrAsset_tuples).all(axis=1))[0]
    intensity[indices] = pcdArrintensity[np.isin(pcdArrAsset_tuples, pcdArr_tuples_np).all(axis=1)]
