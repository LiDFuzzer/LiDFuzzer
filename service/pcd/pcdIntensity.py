...
from domain.semanticMapping import instancesVehicle, instancesPerson, instancesTrfficSign
...
def intensityChange(intensityAsset, details, intensityMod, type):
    ...
    if (type in instancesVehicle.values()):    
        ...
        intensityAsset = np.clip(intensityAsset, 0, 1)

    elif type in instancesPerson.values() or type in instancesTrfficSign.values():
        average = np.average(intensityAsset[mask])
        mod = random.uniform(config.INTENSITY_MIN_CHANGE, config.INTENSITY_MAX_CHANGE)
        if average > .1:
            mod = mod * -1
