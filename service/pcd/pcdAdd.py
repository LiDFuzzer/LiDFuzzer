...
"""
def addInstance(pcdArr, intensity, semantics, labelInstance, pcdArrAsset, details, rotation, modelPredictionsScene, baseSequence):
    attempts = 0
    success = False
    degrees = []
    ...
    while (len(degrees) > 0 and attempts < config.ADD_PLACEMENT_ATTEMPT_LIMIT and not success):
        ...
        mask_list = config.rule[baseSequence][learning_map_inv[details["typeNum"]]]
...
# --------------------------------------------------------------------------
def addInstance_no_rule(pcdArr, intensity, semantics, labelInstance, pcdArrAsset, details, rotation, modelPredictionsScene):
    attempts = 0
    success = False
    degrees = []
...
def addInstance_no_render(pcdArr, intensity, semantics, labelInstance, pcdArrAsset, details, rotation, modelPredictionsScene, baseSequence):
    attempts = 0
    success = False
    degrees = []
    ...
    while (len(degrees) > 0 and attempts < config.ADD_PLACEMENT_ATTEMPT_LIMIT and not success):
        ...
        # Rotate the asset
        pcdArrAssetNew = pcdCommon.rotatePoints(pcdArrAsset, rotateDeg)
        mask_list = config.rule[baseSequence][details["typeNum"]]
        maskGround = np.zeros_like(semantics, dtype=bool)
        for mask in mask_list:
            maskGround |= (semantics == mask)
        # # Check on ground
