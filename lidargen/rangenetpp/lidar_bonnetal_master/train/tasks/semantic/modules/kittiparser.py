...
class FRDPointCloud(Dataset):
  def __init__(self, root,    # directory where data is
    ...
    full_list = sorted(glob('{dump}/*.bin'.format(dump=dump)))
  ...
  def __getitem__(self, index):
    ...
    # open and obtain scan
    # scan.set_points(np.load(scan_file, allow_pickle=True))
    print(scan_file)
    pcdArr = np.fromfile(scan_file, dtype=np.float32)
    print(pcdArr)
    pcdArr = pcdArr.reshape((int(np.shape(pcdArr)[0]) // 4, 4))
    pcdArr = np.delete(pcdArr, 3, 1)
    scan.set_points(pcdArr)
