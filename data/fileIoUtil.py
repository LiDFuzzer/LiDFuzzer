...
def savemodifySemantics(semantics, instances, saveLabelPath, fileName):
    labelsCombined = (instances << 16) | (semantics & 0xFFFF)
...
def openLabelBin(pathVel, pathLabel, sequence, scene):
    ...
    binFile = currPathVel + "/compress_bin/" + scene + ".bin"
    labelFile = currPathLbl + "/compress_label/" + scene + ".label"
    return openLabelBinFiles(binFile, labelFile)

def openInstancesLabelBin(pathVel, pathLabel, sequence, scene):

    folderNum = str(sequence).rjust(2, '0')
    currPathVel = pathVel + folderNum
    currPathLbl = pathLabel + folderNum

    binFile = currPathVel + "/compress_bin(old)/" + scene + ".bin"
    labelFile = currPathLbl + "/compress_label(old)/" + scene + ".label"
...
def openInstanceLabel(pathLabel, sequence, scene):
...
def openPreLabelFile(labelFile, indice, weather_type):
    # Label
    label_arr = np.fromfile(labelFile, dtype=np.int32)
    semantics = label_arr & 0xFFFF
    instances = label_arr >> 16 
    ...
    if weather_type == "rain":
        indice = indice[indice < len(semantics)]
        semantics = semantics[indice]
        instances = instances[indice]
...
def openModelPredictions(modelPath, model, sequence, scene):
    ...
    folderNum = str(sequence).rjust(2, '0')
    predFile = modelPath + "/" + folderNum + "/" + model + "/compress_predlabel/" + scene + ".label"
