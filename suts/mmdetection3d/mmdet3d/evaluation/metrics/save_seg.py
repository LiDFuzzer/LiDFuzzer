...
class SemantickInferMertric(BaseMetric):
    ...
    def process(self, data_batch: dict, data_samples: Sequence[dict]) -> None:
        ...
        # label_name = data_samples[0]["lidar_path"]
        # filename = osp.basename(label_name)
        # filename_number = osp.splitext(filename)[0]
        pred = data_samples[0]['pred_pts_seg']['pts_semantic_mask'] #labels
        map_inv = self.dataset_meta['label_mapping'] #inv mapping
        # pred[pred == 19] += 99 #unlabeled or ignore
        pred += 1 #[0,18] -> [1,19]
        # for i in map_inv: 
        #     pred[pred==i] = map_inv[i]+1000  #avoid the pred label in [1-19] be mapped twice 
        # pred[pred!=119]-=1000
        pred[pred==20] = 0 #unlabel 
        pred.cpu().numpy().astype(np.int32).tofile(f'/home/zjx/semantickitti/sequences/08/prediction/{self.scene_id:06}.label')
        print(f'finsh the /home/zjx/semantickitti/sequences/08/prediction/{self.scene_id:06}.label')
        self.scene_id += 1
