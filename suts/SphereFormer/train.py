...
def validate_distance(val_loader, model, criterion):
    ...
    for i, batch_data in enumerate(val_loader):
        ...
        # pred = output + 1
        # pred[pred==20] = 0
        # pred.cpu().numpy().astype(np.int32).tofile(f'/bdata/usrdata/zjx/semantickitti/sequences/08/prediction/{label_num:06}.label')
        # print(f'finsh the /bdata/usrdata/zjx/semantickitti/sequences/08/prediction/{label_num:06}.label')
        # label_num += 1
        n = coord.size(0)
        if args.multiprocessing_distributed:
            loss *= n
