...
def performMutation(mutation, assetRepo, sessionManager, removeIterateNum):
    ...
    try:
        ...
        if (assetLocation == mutationsEnum.AssetLocation.ADD.name):
            ...
            if (assetRecord != None):
                # Get model prediction (asset & scene)                #获得所有实例
                _, instanceAssetScene = fileIoUtil.openLabel(sessionManager.labelPath, assetRecord["sequence"], assetRecord["scene"])
                for model in sessionManager.models:#打开预测的label以及需要添加的实例的label                    #获取预测的被添加场景的语义以及预测的实例的场景的语义
                    ...
                    modelPredictionsAsset[model] = modelAssetScene[instanceAssetScene == assetRecord["instance"]]#获取预测的实例的语义
...
def runMutations(sessionManager):
    ...
    for mutation in sessionManager.mutationsEnabled:
        if mutation not in finalData.finalData["mutations"]:
            finalData.finalData["mutations"].append(mutation.name)
